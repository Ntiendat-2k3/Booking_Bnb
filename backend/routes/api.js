...
router.patch("/v1/users/me", authMiddleware, csrfMiddleware, validate(updateProfileSchema), accountController.updateProfile);
router.post("/v1/users/me/change-password", authMiddleware, csrfMiddleware, validate(changePasswordSchema), accountController.changePassword);
router.post(
  "/v1/users/me/avatar",
  authMiddleware,
  csrfMiddleware,
...
router.patch("/v1/users/me/settings", authMiddleware, csrfMiddleware, validate(updateSettingsSchema), accountController.updateSettings);
...
// Host dashboard & onboarding
const hostController = require("../controllers/api/v1/host.controller");
router.get("/v1/host/dashboard", authMiddleware, requireRole(["admin", "host"]), hostController.getDashboardStats);
...
router.get("/v1/admin/dashboard/stats", authMiddleware, requireRole(["admin"]), adminDashboardController.getStats);
...
router.post("/v1/admin/listings/bulk-approve", authMiddleware, requireRole(["admin"]), csrfMiddleware, adminListingController.bulkApprove);
router.post("/v1/admin/listings/bulk-reject", authMiddleware, requireRole(["admin"]), csrfMiddleware, adminListingController.bulkReject);
...
router.post("/v1/admin/reviews/bulk-hide", authMiddleware, requireRole(["admin"]), csrfMiddleware, adminReviewController.bulkHide);
router.post("/v1/admin/reviews/bulk-unhide", authMiddleware, requireRole(["admin"]), csrfMiddleware, adminReviewController.bulkUnhide);
router.post("/v1/admin/reviews/bulk-delete", authMiddleware, requireRole(["admin"]), csrfMiddleware, adminReviewController.bulkRemove);
...
router.post(
  "/v1/bookings/:id/payments/stripe",
  authMiddleware,
  csrfMiddleware,
  paymentController.createStripe,
);

router.post("/v1/payments/stripe/webhook", paymentController.stripeWebhook);

// --- [THÔNG BÁO] ---
const NotificationController = require("../controllers/api/v1/notification.controller");
router.get("/v1/notifications/me", authMiddleware, NotificationController.getMyNotifications);
router.patch("/v1/notifications/:id/read", authMiddleware, NotificationController.markAsRead);
router.patch("/v1/notifications/read-all", authMiddleware, NotificationController.markAllAsRead);

// --- [Liên hệ Host] ---
const hostContactController = require("../controllers/api/v1/host.controller");
router.post("/v1/hosts/:id/contact", hostContactController.contactHost);
